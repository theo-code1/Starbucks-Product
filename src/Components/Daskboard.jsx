...
const Dashboard = () => {
    const xWindow = window.innerWidth;
    const yWindow = window.innerHeight;
    const trueSize = window.matchMedia("(max-width: 768px)").matches;
    return (
        <section className="dashboard lg:h-screen h-[120dvh] pt-4 bg-gradient-to-r from-green to-light-green relative overflow-hidden ">
           {trueSize && <div className="phone-message w-screen h-screen bg-green z-[100]">
                <div className="text-center text-white font-bold text-4xl h-full flex items-center justify-center">This page is not available on mobile devices</div>
            </div>}
            <nav className="navBar w-[95%] px-4 mx-auto h-[8vh] flex items-center justify-between text-white z-40 animate-go-down ">
            ...
            <div className="left-b-text text-white gap-0 h-auto w-[400px] absolute lg:bottom-8 md:flex flex-col sm:hidden left-6 animate-go-right">
            ...
            <div className="right-b-text w-[400px] absolute lg:bottom-8 lg:flex md:hidden sm:hidden right-6 text-white animate-go-left">
            ...
            <p className='bg-blue-600 text-white text-2xl absolute top-6 right-8'>height :{yWindow}</p>
            <p className='bg-blue-600 text-white text-2xl absolute top-16 right-8'>Width :{xWindow}</p>
        </section>
    );
}
